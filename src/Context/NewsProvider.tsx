...
export const NewsProvider = ({children}:{children: React.ReactNode}) =>{
    ...
    const allNewsData = async()=> {
        try{
        setLoading(true)
        const res = await axios.get(`${import.meta.env.VITE_API_URL}/News/readallnews/allProduct`)
...
const categoriesNewsData = async(categorynews:string , page = 1)=>{
    try{
        ...
        const CategoryResponse = await axios.get(`${import.meta.env.VITE_API_URL}/News/readProduct/${encodeURIComponent(categorynews)}`,{
